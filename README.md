...
```text
         ┌──────────────────────────────────────────┐
         │                                          │
   LED(PA7=TIM3_CH2) ──光照──▶ 光敏传感器 ──ADC──▶ PI 控制器(10Hz)
   1kHz PWM 占空比 0-999        (ADC 越高=越暗)    死区+积分限幅
         ▲                                          │
         └────────── CCR2 占空比 ◀───────────────────┘
```

- **硬件零改动**：状态指示 LED（PA7，恰好是 TIM3_CH2 复用脚）升级为执行器
- **TIM3 一鱼两吃**：1Hz 统计时基重构为 1kHz PWM 载波，统计回调改千分频
- **与状态闪灯零冲突**：调光开启时 PA7 切复用模式，ODR 与引脚脱开，Task_LED
  的 TogglePin 写的是无人消费的寄存器；关闭调光即恢复指示灯功能
- **工程细节**：带死区（频带内输出保持，抗传感器噪声极限环）、积分限幅
  （防饱和）、单向约束（LED 只能加光，duty 下限 0）
- 上位机：`python3 tools/ota_tool.py dim /dev/ttyUSB0 on [--target 1100]`
...
共 22 个高难度 Bug（#1-#15 见下表，#16-#22 含 fuzz/模型检查/真板发现的信任链与状态机缺陷），完整调试过程记录在 [BUGS.md](BUGS.md)：
